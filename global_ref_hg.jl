...
function main()
	...
	dimX1, dimX2, dimA = (7, 7, 5)
	dimX = dimX1*dimX2
	dimG = 40
	dimH = 3
	global dims = (dimX1, dimX2, dimA, dimG, dimH)
	...
	C    = CreateWalls(dims, "R")
	G    = GenerateGoalSet(dims, "fullX", C)
# 	dimG = length(G)
# 	println(dimG)
# 	println(dims)
# 	dims = (dims[1], dims[2], dims[3], dimG, dims[5])
	...
	for Pg_idx in 1
		...
		Pg      	= eps(Float16)*ones(dimG, 1, 1, 1, 1, 1)
		Pg[Pg_idx] 	= 1.0
		Pg = Normalize(Pg, (idxG,))
		...
		Pgxi_ai[  :, 1, :, :, 1, 1] = Pgxi_ai[ :, 1, :, :, 1, 1]  + 0.001*rand(dimG, dimX, dimA)
